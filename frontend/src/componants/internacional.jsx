...
function Internacional() {
  ...
  const [internacionais, setInternacionais] = useState([]);
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/internacionais/')
            .then((response) => {
                setInternacionais(response.data);
                setLoading(false);
            })
            .catch((error) => {
                console.error(error);
    ...
    const handleDelete = (internacionalId) => {
      // Send a DELETE request to your API to delete the Avião
      axios.delete(`http://localhost:8000/api/internacionais/${internacionalId}/`)
        .then(() => {
          // Update the state to reflect the deletion
          setInternacionais(internacionais.filter((internacional) => internacional.id !== internacionalId));
        })
        .catch((error) => {
    ...
    return (
        <div className="container-fluid">
          <h3 className="m-4">Voos Internacionais</h3>
          <div className="row">
            <div className="col-12">
              ...
              {loading ? (
                ...
                <div className="card bg-light ms-4 me-4 mb-4">
                  ...
                  <h5><FontAwesomeIcon icon={faTableList}/> Lista de Voos Internacionais</h5>
                  </div>
                ...
                <h5 className="alert alert-primary ms-4 me-4">Sem Registros de Voos Internacionais</h5>
              )}
            </div>
          </div>
        </div>
